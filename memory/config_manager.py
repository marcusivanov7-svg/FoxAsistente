...
def get_assistant_name() -> str:
    """Return the configured assistant name, or 'FOX' if not set."""
    return load_api_keys().get("assistant_name", "FOX") or "FOX"
...
def save_assistant_config(assistant_name: str, user_name: str) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            data = {}
    data["assistant_name"] = assistant_name.strip() or "FOX"
...
# ── Model routing (AMSY) ─────────────────────────────────────────────────────
# model_for_conversation / model_for_agents / model_for_search / model_for_vision
# are opt-in overrides. They only take effect when they hold a concrete model id
# (contains '/', or starts with 'gemini-'/'models/'); a bare provider name like
# 'gemini' falls back to the per-module default so nothing breaks.
def get_model_for(kind: str, default: str) -> str:
    val = (load_api_keys().get(f"model_for_{kind}") or "").strip()
    if not val:
        return default
    if "/" in val or val.startswith("gemini-") or val.startswith("models/"):
        return val
    return default