...
# ── Token-saver mode (AMSY) ──────────────────────────────────────────────────
# When on, the prompt rides a tighter memory budget so the Live session connects
# with fewer tokens → faster first token. The full store stays on disk and is
# fetched via recall_memory (local, instant). Set from config by main.py.
TOKEN_SAVER_MODE = False

def set_token_saver(enabled: bool) -> None:
    global TOKEN_SAVER_MODE
    TOKEN_SAVER_MODE = bool(enabled)
...
def format_memory_for_prompt(memory: dict | None) -> str:
    ...
    _core = 600 if TOKEN_SAVER_MODE else PROMPT_CORE_CHARS
    _idx  = 300 if TOKEN_SAVER_MODE else PROMPT_INDEX_CHARS
...
def _score(query_words: list[str], cat: str, key: str, value: str) -> int:
    ...
    for w in query_words:
        if not w:
            continue
        w = _pretty(w).lower()   # "favorite_color" -> "favorite color" (igual que las keys)
        if w == hay_key:
            score += 10
        elif w in hay_key:
            score += 6
...
def all_entries_for_ui() -> list[dict]:
    """Flat list for the memory panel: what FOX knows, and when it learned it.
