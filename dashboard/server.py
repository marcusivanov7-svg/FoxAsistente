"""
dashboard/server.py — FOX Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "FOX Uploads",
        Path.home() / "Documents" / "FOX Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"FOX Dashboard Port {port}"
        prog_rule  = "FOX Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="fox_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until FOX is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "FOX Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "FOX"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "fox.key").exists() and (certs / "fox.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in FOX to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('fox_token','{tok}');
  sessionStorage.setItem('fox_key','{key}');
  localStorage.setItem('fox_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to FOX…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "fox.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "fox.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in FOX UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
