...
# Sube/baja el volumen de UNA app concreta (Chrome, Spotify, Discord...) sin
# tocar el volumen general. Usa pycaw (ISimpleAudioVolume), igual que el Fox
# original. `relative` sube/baja `value` puntos porcentuales en vez de fijarlo.
def set_app_volume(app_name: str, value: int, relative: bool = False) -> bool:
    if _OS != "Windows":
        return False
    try:
        ...
        for session in sessions:
            ...
            if target and (target in pname or pname in target):
                ...
                if relative:
                    ...
                    volume.SetMasterVolume(max(0.0, min(1.0, value / 100.0)), None)
                found = True
        return found
    except Exception as e:
        print(f"[Settings] Error ajustando volumen de app: {e}")
        return False
# ──────────────────────────────────────────────────────────────────────────────
...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            # Fallback real: emula las teclas de volumen hasta acercarse al valor.
            # Es ciego, pero no miente: no reporta éxito si no pudo aplicar pycaw.
            raise RuntimeError(f"No se pudo fijar el volumen por pycaw: {e}")
    elif _OS == "Darwin":
...
def _focus_browser() -> None:
    """Trae al frente la ventana del navegador (Chrome/Edge/Firefox/Opera/Brave)
    para que los atajos de pestañas vayan al navegador y no a Fox/otra app."""
    _browsers = ("chrome", "edge", "firefox", "opera", "brave", "vivaldi")
    try:
        _wins = pyautogui.getAllWindows()
    except Exception:
        return
    for _w in _wins:
        try:
            _title = (_w.title or "").lower()
        except Exception:
            continue
        if any(_b in _title for _b in _browsers):
            try:
...
def close_tab_n(n: int):
    """Cierra la pestaña N del navegador: Ctrl/Cmd+N salta a esa pestaña
    (Ctrl/Cmd+9 = última) y después la cierra. Chrome/Edge/Firefox."""
    n = max(1, min(9, int(n)))
    _focus_browser()
    if _OS == "Darwin":
        pyautogui.hotkey("command", str(n))
    else:
        pyautogui.hotkey("ctrl", str(n))
    time.sleep(0.35)  # dejar que el navegador cambie de pestaña antes de cerrar
    close_tab()
...
_IRREVERSIBLE = {
    ...
    "toggle_wifi": ("Switch WiFi off or on",
                    "If this switches WiFi off, FOX loses its connection and "
...
def computer_settings(
    ...
    target_app  = str(params.get("target_app", "") or "").strip().lower()

    # Respaldo: si el modelo olvida "target_app", lo buscamos en "description".
    if not target_app and description:
        desc_lower = description.lower()
        for _name in ("chrome", "spotify", "edge", "msedge", "firefox",
                      "discord", "steam", "youtube", "opera", "brave", "vivaldi"):
            if _name in desc_lower:
                target_app = "msedge" if _name in ("edge", "msedge") else _name
                break
    ...
    # ── Volumen por aplicación (si se nombró una app concreta) ───────────────
    # "bajale el volumen a Spotify", "silenciá Chrome", "subile a Discord"…
    if target_app and action in (
        "volume_set", "volume", "set_volume", "volume_up", "volume_down",
        "mute", "unmute", "toggle_mute",
    ):
        try:
            if action in ("mute", "unmute", "toggle_mute"):
                ok = set_app_volume(target_app, 0)
                return f"{target_app} silenciado." if ok else f"No se encontró {target_app} reproduciendo audio."
            if action in ("volume_up", "volume_down"):
                delta = 10 if action == "volume_up" else -10
                ok = set_app_volume(target_app, delta, relative=True)
                return (f"Volumen de {target_app} {'subido' if action == 'volume_up' else 'bajado'}."
                        if ok else f"No se encontró {target_app} reproduciendo audio.")
            # volume_set / volume: valor absoluto 0-100
            target = int(float(str(value).replace("%", "")) if value is not None else 50)
            ok = set_app_volume(target_app, target)
            return (f"Volumen de {target_app} configurado al {target}%."
                    if ok else f"No se encontró {target_app} reproduciendo audio.")
        except Exception as e:
            return f"Error ajustando el volumen de {target_app}: {e}"
    ...
    if action == "close_tab":
        ...
        if n is not None and 1 <= n <= 9:
            close_tab_n(n)
            return f"Closed tab {n}."
        close_tab()
        return "Done: close_tab."
...
TOOL = {
    ...
    "description": "Controls the computer HARDWARE and OS: volume, brightness, window management, keyboard shortcuts, typing text on screen, closing apps, fullscreen, dark mode, WiFi, restart, shutdown, scrolling, tab management, zoom, screenshots, lock screen, refresh/reload page. To close a SPECIFIC browser tab (e.g. 'cerrá la tercera pestaña'), use action=close_tab with value='3' (1=first tab … 9=last tab); close_tab without value closes the current tab. Use for ANY single computer control command. shutdown and restart here mean the COMPUTER/PC (the machine), NOT the assistant. If the user asks to close/exit/stop the assistant itself ('apagate', 'cerrate', 'cerrá el asistente'), use shutdown_fox instead. restart, shutdown and toggle_wifi put a confirmation on the user's screen and do NOT happen until they press it — never claim they are done. Volume, brightness and dark mode can be reversed with the `undo` tool.",
    "parameters": {   'type': 'OBJECT',
        'properties': {   'action': {   'type': 'STRING',
                                        'description': 'The exact action. Prefer '
                                                       'this over `description` — '
                                                       'pick one of: volume_up | '
                                                       'volume_down | volume_set | '
                                                       'mute | brightness_up | '
                                                       'brightness_down | '
                                                       'sleep_display | '
                                                       'pause_video | close_app | '
                                                       'close_window | full_screen '
                                                       '| minimize | maximize | '
                                                       ...
                                                       'task_manager | '
                                                       'focus_search | '
                                                       'refresh_page | close_tab | '
                                                       'new_tab | next_tab | '
                                                       'prev_tab | go_back | '
                                                       'go_forward | zoom_in | '
                                                       'zoom_out | zoom_reset | '
                                                       'find_on_page | scroll_up | '
                                                       'scroll_down | scroll_top | '
                                                       'scroll_bottom | page_up | '
                                                       'page_down | copy | paste | '
                                                       'cut | undo | redo | '
                                                       'select_all | save | enter '
                                                       '| escape | press_key | '
                                                       'type_text | screenshot | '
                                                       'lock_screen | '
                                                       'open_settings | '
                                                       'file_explorer | open_run | '
                                                       'dark_mode | toggle_wifi | '
                                                       'restart | shutdown'},
                          'description': {   'type': 'STRING',
                                             'description': 'Fallback only, when '
                                                            'no action name above '
                                                            'fits. Resolved '
                                                            'locally — no extra '
                                                            'model call.'},
                          'value': {   'type': 'STRING',
                                       'description': 'Optional value: volume '
                                                      'level 0-100, text to type, '
                                                      'key name, or tab number 1-9 '
                                                      'for close_tab.'},
                          'target_app': {   'type': 'STRING',
                                            'description': 'App whose volume to '
                                                           'change, for volume '
                                                           'actions only (e.g. '
                                                           "'chrome', 'spotify', "
                                                           "'edge', 'discord', "
                                                           "'firefox'). When set, "
                                                           'volume_set/volume_up/volume_down/mute '
                                                           'act on that single app '
