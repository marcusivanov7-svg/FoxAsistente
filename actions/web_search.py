...
from memory.config_manager import get_model_for
...
def _get_tavily_key() -> str:
    try:
        with open(API_CONFIG_PATH, "r", encoding="utf-8") as f:
            return (json.load(f).get("tavily_api_key") or "").strip()
    except Exception:
        return ""


def _tavily_search(query: str) -> str:
    """Busca en Tavily (API de búsqueda diseñada para IA) y devuelve la respuesta
    sintetizada si existe, o los resultados formateados. Devuelve '' si no está
    configurada o falla — el caller cae a Gemini/DDG."""
    key = _get_tavily_key()
    if not key:
        return ""
    ...
    try:
        ...
        if results:
            return "\n".join(
                f"{i + 1}. {res.get('title', '')}: {res.get('content', '')}"
                for i, res in enumerate(results[:5])
            )
    except Exception as e:
        print(f"[WebSearch] Tavily failed ({e}) — using Gemini/DDG")
    return ""


def _gemini_search(query: str) -> str:
    # Tavily primero (si está configurada): da respuesta sintetizada sin gastar
    # la cuota de grounding de Gemini. Si no hay key o falla, seguimos.
    _tav = _tavily_search(query)
    if _tav:
        return _tav
    ...
    try:
        response = client.models.generate_content(
            model=get_model_for("search", "gemini-flash-latest"),
            contents=query,
            config={
                "tools": [{"google_search": {}}],
                # evita el warning "Direct use of automatic function calling"
...
def _ddg_news(query: str, max_results: int = 8) -> list[dict]:
    """DDG news search — devuelve noticias FRESCAS (hoy); si no hay, sin filtro.

    Sin el filtro de fecha, DDG repetía los mismos titulares "evergreen" y el
    briefing leía noticias desactualizadas todos los días."""
    DDGS = _get_ddgs()

    def _fetch(timelimit):
        out = []
        with DDGS() as ddgs:
            for r in ddgs.news(query, timelimit=timelimit, max_results=max_results):
                out.append({
                    "title":   r.get("title",  ""),
                    "snippet": r.get("body",   ""),
                    "url":     r.get("url",    ""),
    ...
    for _tl in ("d", None):
        try:
            ...
            print(f"[WebSearch] ⚠️ DDG news() failed ({e}) — sin resultados de noticias")
    # Sin resultados de noticias devolvemos vacío, NO texto de búsqueda web:
    # los títulos truncados ("Médicos paragu…") confunden al modelo y lo hacen
    # reintentar la búsqueda en bucle (y el scraping de texto es lo que se colgaba 5s).
    return []
...
def _news(query: str) -> str:
    ...
    text = _run_bounded(_ddg_attempt, timeout=3.0, label="DDG news")
...
TOOL = {
    ...
    "parameters": {   'type': 'OBJECT',
        'properties': {   'query': {   'type': 'STRING',
                          ...
                          'items': {   'type': 'ARRAY',
                                       'items': {'type': 'STRING'},
                                       'description': 'Items to compare (compare '
                                                      'mode)'},
                          'aspect': {   'type': 'STRING',
                                        'description': 'Comparison aspect: price | '
                                                       'specs | reviews | '
                                                       'features'}},
        'required': ['query']},
    "handler": web_search,
