...
def _build_google_flights_url(
    ...
    # Estado real de la búsqueda en el fragmento `#flt=` (fechas REALES).
    # Antes había un token `tfs` hardcodeado con 2025-03-15 → "fecha pasada".
    legs = f"{origin}.{destination}.{date}"
    if return_date:
        legs += f"*{destination}.{origin}.{return_date}"
    fragment = f"#flt={legs};c:USD;e:{cabin_code};sd:{passengers};t:f"

    return f"{base}?q={trip}&curr=USD&hl=en{fragment}"
    ...
    "parameters": {   'type': 'OBJECT',
        'properties': {   'origin': {   'type': 'STRING',
                          ...
                          'destination': {   'type': 'STRING',
                                             'description': 'Arrival city or '
                                                            'airport code'},
                          'date': {   'type': 'STRING',
                                      'description': 'Departure date (any format)'},
                          'return_date': {   'type': 'STRING',
                                             'description': 'Return date for round '
                                                            'trips'},
                          'passengers': {   'type': 'INTEGER',
                                            'description': 'Number of passengers '
                                                           '(default: 1)'},
                          'cabin': {   'type': 'STRING',
                                       'description': 'economy | premium | '
                                                      'business | first'},
                          'save': {   'type': 'BOOLEAN',
