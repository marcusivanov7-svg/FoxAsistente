...
_COMMON_TLDS = {
    "com", "net", "org", "io", "es", "ar", "py", "cl", "mx", "co", "dev",
    "ai", "app", "gg", "tv", "me", "info", "biz", "site", "online", "edu",
    "gov", "com.ar", "com.py", "co.py", "com.uy", "com.mx",
}

def _as_url(raw: str) -> str | None:
    """Si `raw` parece una URL o dominio, devuelve una URL http(s); si no, None."""
    s = raw.strip()
    if not s:
        return None
    low = s.lower().rstrip("/")
    ...
    if re.match(r"^[a-z0-9-]+(\.[a-z0-9-]+)+$", low):
        parts = low.split(".")
        if parts[-1] in _COMMON_TLDS or ".".join(parts[-2:]) in _COMMON_TLDS:
            return "https://" + s
    return None
...
def open_app(
    ...
    # URL/dominio → abrir en el navegador por defecto (reutiliza Chrome abierto).
    _url = _as_url(app_name)
    if _url:
        print(f"[open_app] URL detected: '{app_name}' → {_url}")
        try:
            _ok = webbrowser.open(_url)
            return f"Opened {_url} in your browser." if _ok else f"Could not open {_url}."
        except Exception as e:
            print(f"[open_app] webbrowser failed: {e}")
            return f"Failed to open {_url}: {e}"
...
TOOL = {
    ...
    "parameters": {   'type': 'OBJECT',
        'properties': {   'app_name': {   'type': 'STRING',
                                          'description': 'Exact name of the '
                                                         'application (e.g. '
                                                         "'WhatsApp', 'Chrome', "
                                                         "'Spotify') or a website "
                                                         'URL/domain (e.g. '
                                                         "'youtube.com', "
                                                         "'gmail.com', "
                                                         "'https://ejemplo.com') — "
                                                         'URLs open in the default '
                                                         'browser.'}},
        'required': ['app_name']},
    "handler": open_app,
}
