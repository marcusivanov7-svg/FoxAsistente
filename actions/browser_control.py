...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".fox_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".fox_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using FOX profile")
                fox = str(Path.home() / ".fox_profiles" / "firefox_fox")
                Path(fox).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(fox, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".fox_profiles" / "safari")
        ...
        # FOX otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        fox_profile = str(Path.home() / ".fox_profiles" / self.browser_name)
        Path(fox_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with FOX profile: {fox_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(fox_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with FOX profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "fox_screenshot.png")
...
TOOL = {
    ...
    "description": "Controls a web browser. Use for: opening websites, searching, filling forms, scrolling, reading page text, screenshots, and multi-step web automation. NOTE: interactive actions (click/type/smart_click) run in a SEPARATE controlled automation browser, NOT the user's visible browser. To click or type on what the user can actually SEE on screen (their real browser or desktop apps), use computer_control with action=screen_click instead. Simple open/search requests launch the user's own browser normally (their real profile and logged-in accounts). Always pass the 'browser' parameter when the user specifies a browser (e.g. 'open in Edge', 'use Firefox', 'open Chrome'). Multiple browsers can run simultaneously.",
    "parameters": {   'type': 'OBJECT',
        'properties': {   'action': {   'type': 'STRING',
                                        'description': 'go_to | search | click | '
                                                       'type | scroll | fill_form '
                                                       '| smart_click | smart_type '
                                                       '| get_text | get_url | '
                                                       'press | new_tab | '
                                                       'close_tab | screenshot | '
                                                       'back | forward | reload | '
                                                       'switch | list_browsers | '
                                                       'close | close_all'},
                          'browser': {   'type': 'STRING',
                                         'description': 'Target browser: chrome | '
                                                        'edge | firefox | opera | '
                                                        'operagx | brave | vivaldi '
                                                        '| safari. Omit to use the '
                                                        'currently active '
                                                        'browser.'},
                          'url': {   'type': 'STRING',
                                     'description': 'URL for go_to / new_tab '
                                                    'action'},
                          'query': {   'type': 'STRING',
                          ...
                          'amount': {   'type': 'INTEGER',
                                        'description': 'Scroll amount in pixels '
                                                       '(default: 500)'},
                          'key': {   'type': 'STRING',
                                     'description': 'Key name for press action '
                                                    '(e.g. Enter, Escape, F5)'},
                          'path': {   'type': 'STRING',
                          ...
                          'clear_first': {   'type': 'BOOLEAN',
                                             'description': 'Clear field before '
                                                            'typing (default: '
                                                            'true)'}},
        'required': ['action']},
