...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = Path.home() / "Desktop" / "fox_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    try:
        ...
        # FOX: la captura sale en píxeles NATIVOS, pero pyautogui mueve/cliquea en
        # coordenadas LÓGICAS. Con DPI scaling son distintas → re-escalamos al final.
        sw, sh = img.size
        lw, lh = pyautogui.size()

        client = genai.Client(api_key=api_key)
        prompt = (
            f"This is a screenshot of a {sw}×{sh} pixel screen. "
            f"Locate the CLICKABLE element (button, link, thumbnail, icon) described as: '{description}'. "
            f"Reply with ONLY the center coordinates as: x,y "
            f"If the element is not visible, reply: NOT_FOUND"
        )

        # FOX: la llamada a Gemini se acota a 10s. Si la cuota está agotada o la
        # red cuelga, sin esto screen_click se congelaba MINUTOS (timeout por
        # defecto del SDK) y bloqueaba el bucle de recepción de Fox.
        _box = [None]

        def _call():
            try:
                _box[0] = client.models.generate_content(
                ...
                print(f"[ComputerControl] ⚠️ screen_find Gemini call failed: {e}")

        _t = threading.Thread(target=_call, daemon=True)
        _t.start()
        _t.join(timeout=10.0)
        if _t.is_alive():
            print("[ComputerControl] ⚠️ screen_find excedió 10s — cancelando el clic")
            return None
        if _box[0] is None:
            return None

        text = (_box[0].text or "").strip()
        ...
        if match:
            x = int(match.group(1))
            y = int(match.group(2))
            # Re-escalar de píxeles nativos a coordenadas lógicas de pyautogui.
            # Sin esto, con DPI scaling el clic caía corrido ("fallaba totalmente").
            if lw and lh and (lw != sw or lh != sh):
                x = int(x * lw / sw)
                y = int(y * lh / sh)
            return x, y
...
def computer_control(
    ...
    try:
        ...
        if action == "screen_click":
            ...
            if coords:
                # FOX: mover el mouse de forma visible hasta el punto y hacer clic
                # (en vez de teletransportarlo), como lo haría un humano.
...
TOOL = {
    ...
    "description": "Direct control of the user's VISIBLE screen — their real web browser, desktop apps and OS UI: type, click, hotkeys, scroll, move mouse, screenshots. screen_click/screen_find use screen vision: take a screenshot, locate the element, move the mouse and click. Use screen_click to click ANYTHING the user can see on screen (including their real browser). Use browser_control only for automation in a separate controlled browser.",
    "parameters": {   'type': 'OBJECT',
        'properties': {   'action': {   'type': 'STRING',
                                        'description': 'type | smart_type | click '
                                                       '| double_click | '
                                                       'right_click | hotkey | '
                                                       'press | scroll | move | '
                                                       'copy | paste | screenshot '
                                                       '| wait | clear_field | '
                                                       'focus_window | screen_find '
                                                       '| screen_click | '
                          ...
                          'keys': {   'type': 'STRING',
                                      'description': 'Key combination e.g. '
                                                     "'ctrl+c'"},
                          'key': {   'type': 'STRING',
                                     'description': "Single key e.g. 'enter'"},
                          'direction': {   'type': 'STRING',
                                           'description': 'up | down | left | '
                                                          'right'},
                          'amount': {   'type': 'INTEGER',
                                        'description': 'Scroll amount (default: '
                                                       '3)'},
                          'seconds': {   'type': 'NUMBER',
                                         'description': 'Seconds to wait'},
                          'title': {   'type': 'STRING',
                                       'description': 'Window title for '
                                                      'focus_window'},
                          ...
                          'field': {   'type': 'STRING',
                                       'description': 'Field for user_data: '
                                                      'name|email|city'},
