"""
office_builder.py — Fox AI office document generation

Creates PowerPoint presentations (.pptx), Excel workbooks (.xlsx) and Word
documents (.docx) from structured inputs. Ported from the original Fox
(actions/office_builder.py + actions/word_controller.py) and adapted to the v2
plugin signature.
"""
...
PROJECT_NAME = "Fox"
DEFAULT_OUTPUT_DIR = Path.home() / "Desktop" / "FoxAI"
...
def _resolve_output_path(output_path: str | None, title: str, ext: str) -> Path:
    ...
    return DEFAULT_OUTPUT_DIR / f"{_sanitize_filename(title, 'fox_ai_output')}{ext}"
...
def _theme_library():
    return {
        "auto": {
            "bg": "07131C", "panel": "0E2230", "accent": "00D4FF", "accent2": "FF8A3D",
            "accent3": "7C5CFF", "text": "F1FAFF", "muted": "8DB7C8", "line": "23485E",
            "glow": "103549", "surface": "0B1D28",
        },
        "neon": {
            "bg": "05070C", "panel": "111827", "accent": "21E6C1", "accent2": "7C5CFF",
            "accent3": "00D4FF", "text": "F6FAFF", "muted": "98A9C0", "line": "253245",
            "glow": "0D2533", "surface": "0B1220",
        },
        "corporate": {
            "bg": "081018", "panel": "102130", "accent": "F97316", "accent2": "22C55E",
            "accent3": "38BDF8", "text": "F8FAFC", "muted": "94A3B8", "line": "2B4057",
            "glow": "132433", "surface": "0F172A",
        },
        "luxury": {
            "bg": "0A0910", "panel": "161320", "accent": "D4AF37", "accent2": "F3E8C7",
            "accent3": "A855F7", "text": "FFFDF7", "muted": "C9C1B2", "line": "3B314E",
            "glow": "191224", "surface": "11111A",
        },
        "academic": {
            "bg": "0D1117", "panel": "141A22", "accent": "60A5FA", "accent2": "F59E0B",
            "accent3": "34D399", "text": "F8FAFC", "muted": "94A3B8", "line": "263445",
            "glow": "102235", "surface": "0F1720",
        },
        "sunset": {
            "bg": "1A0F14", "panel": "25141A", "accent": "FB7185", "accent2": "FDBA74",
            "accent3": "A78BFA", "text": "FFF7F8", "muted": "E5B7C0", "line": "402430",
            "glow": "2A1320", "surface": "1E1016",
        },
    }
...
def _slides_from_outline(outline: str | None, provided: list[dict] | None) -> list[dict]:
    ...
    if not outline:
        return [
            {"title": "RESUMEN", "bullets": ["Add a slide outline or structured slides input."]},
        ]
...
def create_presentation(parameters: dict, player=None) -> str:
    ...
    for idx, spec in enumerate(slides, 1):
        ...
        if layout == "section":
            ...
            if bullets:
                add_bullet_card(slide, Inches(7.55), Inches(1.92), Inches(4.0), Inches(2.6), "PUNTOS CLAVE", bullets, accent_key=accent_key, dense=True)
            add_textbox(slide, Inches(0.86), Inches(6.05), Inches(7.8), Inches(0.3),
                        spec.get("notes") or "Divisor de sección", font_size=10, color_key="muted")
            continue

        if layout == "split":
            add_section_header(slide, slide_title, kicker, Inches(0.8), Inches(0.62), Inches(8.6), accent_key=accent_key)
            add_bullet_card(slide, Inches(0.82), Inches(1.62), Inches(6.0), Inches(4.95), "IDEAS PRINCIPALES", bullets or ["Add supporting content."], accent_key=accent_key)
            ...
            add_textbox(slide, Inches(7.5), Inches(1.92), Inches(4.0), Inches(0.4), "DESTACADOS", font_size=14, color_key=accent_key, bold=True, all_caps=True)
            ...
            for item in highlight:
                ...
                y += 0.58
            add_textbox(slide, Inches(7.52), Inches(5.9), Inches(4.0), Inches(0.35), spec.get("notes") or "Llamado visual", font_size=10, color_key="muted")
            continue

        add_section_header(slide, slide_title, kicker, Inches(0.8), Inches(0.62), Inches(8.6), accent_key=accent_key)
        add_bullet_card(slide, Inches(0.82), Inches(1.58), Inches(7.2), Inches(4.9), "RESUMEN", bullets or ["Add supporting content."], accent_key=accent_key)
        add_kpi(slide, Inches(8.35), Inches(1.68), Inches(3.05), Inches(0.95), "ESTADO", spec.get("ESTADO") or "Listo", accent_key=accent_key)
...
def create_document(parameters: dict, player=None) -> str:
    ...
    output_path = _resolve_output_path(
        parameters.get("output_path") or parameters.get("file_name"), title, ".docx"
    )
    auto_open = parameters.get("auto_open", True)
    font_name = (parameters.get("font_name") or "Aptos").strip()
    try:
        font_size = int(parameters.get("font_size", 11))
    except Exception:
        font_size = 11
    ...
    if content:
        for line in str(content).splitlines():
            ...
            # Normal paragraph with **bold** support
                ...
                p = doc.add_paragraph()
                for i, parte in enumerate(line.split("**")):
                    run = p.add_run(parte)
                    if i % 2 != 0:
                        run.bold = True
...
def office_builder(parameters=None, player=None, speak=None, **kwargs) -> str:
    """Dispatcher for the v2 tool. `action` decides the document type."""
    p = parameters or {}
    action = str(p.get("action") or p.get("type") or "").strip().lower()

    if action in ("presentation", "ppt", "pptx", "slides", "create_presentation", "powerpoint"):
        return create_presentation(p, player)
    if action in ("spreadsheet", "excel", "xlsx", "sheet", "sheets", "create_spreadsheet"):
        return create_spreadsheet(p, player)
    if action in ("document", "word", "docx", "create_document", "text"):
        return create_document(p, player)

    return (
        "Unknown office action. Use action='presentation' (create a .pptx), "
        "action='spreadsheet' (create a .xlsx) or action='document' (create a .docx)."
    )

TOOL = {
    "name": 'office_builder',
    "description": "Creates Office documents and saves them: PowerPoint presentations (.pptx), Excel spreadsheets (.xlsx) and Word documents (.docx). Use whenever the user asks to make, create, armá, generá a presentation, a spreadsheet/table of data, or a Word/text document with structured content. Pick `action` by document type: 'presentation' | 'spreadsheet' | 'document'. For presentations pass a title and an outline (one slide per blank line, first line is the slide title, following lines are its bullets), or `slides` as a JSON array. For spreadsheets pass `title` and `worksheets` as a JSON array of {name, headers, rows} (or use `headers`/`rows` for a single sheet). For Word documents pass `title` and `content` (use # for headings, - for bullets). The file is saved to the Desktop (FoxAI folder) by default and opened automatically.",
    "parameters": {   'type': 'OBJECT',
        'properties': {   'action': {   'type': 'STRING',
                                        'description': 'presentation | spreadsheet '
                                                       '| document'},
                          'title': {   'type': 'STRING',
                                       'description': 'Document/file title (also '
                                                      'used as the file name)'},
                          'subtitle': {   'type': 'STRING',
                                          'description': 'Presentation subtitle '
                                                         '(presentation only)'},
                          'theme': {   'type': 'STRING',
                                       'description': 'Presentation theme hint: '
                                                      'auto | neon | corporate | '
                                                      'luxury | academic | sunset'},
                          'outline': {   'type': 'STRING',
                                         'description': 'Presentation outline: one '
                                                        'slide per blank line; '
                                                        'first line = slide title, '
                                                        'next lines = bullets'},
                          'slides': {   'type': 'ARRAY',
                                        'items': {'type': 'OBJECT'},
                                        'description': 'Optional explicit slide '
                                                       'specs [{title, bullets}]'},
                          ...
                          'headers': {   'type': 'ARRAY',
                                         'items': {'type': 'STRING'},
                                         'description': 'Single-sheet headers'},
                          'rows': {   'type': 'ARRAY',
                                      'items': {   'type': 'ARRAY',
                                                   'items': {'type': 'STRING'}},
                                      'description': 'Single-sheet rows'},
                          'content': {   'type': 'STRING',
                                         'description': 'Word document content (# '
                                                        'heading, ## subheading, - '
                                                        'bullet)'},
                          ...
                          'output_path': {   'type': 'STRING',
                                             'description': 'Optional '
                                                            'full/relative output '
                                                            'path (e.g. '
                                                            "'desktop/informe.pptx')"},
                          'auto_open': {   'type': 'BOOLEAN',
                                           'description': 'Open the file after '
                                                          'creating it (default: '
                                                          'true)'}},
        'required': ['action']},
    "handler": office_builder,
}
