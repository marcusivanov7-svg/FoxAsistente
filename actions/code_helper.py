...
GEMINI_MODEL       = get_model_for("agents", "gemini-flash-latest")
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"fox_debug_{int(time.time())}.png"
...
TOOL = {
    ...
    "parameters": {   'type': 'OBJECT',
        'properties': {   'action': {   'type': 'STRING',
                                        'description': 'write | edit | explain | '
                                                       'run | build | auto '
                                                       '(default: auto)'},
